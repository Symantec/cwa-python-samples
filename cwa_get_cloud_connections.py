...
# On success, this script will produce cloud connection ids and their names.
...
# create logger
logger = logging.getLogger("cwa_get_cloud_connections")
...
GET_CONNECTION='GetCloudConnections'

# Getting current working directory
Current_Working_Dir = os.getcwd()

logger.info("cwa_get_cloud_connections: Current working Directory is " + Current_Working_Dir)
logger.info("cwa_get_cloud_connections: Checking if config file present in current directory")

config_file = Path(Current_Working_Dir + '/config.ini')

# Checking if config file present
if not config_file.is_file():
    logger.error("cwa_get_cloud_connections: File config.ini not found in current working directory, place config.ini "
                 "fle in directory \n " + Current_Working_Dir)
    exit()
else:
    logger.info("cwa_get_cloud_connections: Configfile found in directory " + Current_Working_Dir)
...
def get_cloud_connections():
    global auth_headers
    get_connection_url = getconnection_url
    ...
    try:
        logger.info("get_cloud_connections(): Hitting http request to get accounts using connection id")
        ...
        if response.status_code  == 200:
          ...
          if records:
            for record in records:
               print("Account Name: "+ record["name"] +" | ID: " +record["id"]+ " | Cloud Platform: "+record["cloud_platform"])
        else:
            error_message = response_json["statusDetailedMessage"]
            logger.error("Error occured while fetching cloud connections, " + error_message)
    except Exception as ex:
                logger.error("get_cloud_connections(): Exception occurred while fetching account " + str(ex))

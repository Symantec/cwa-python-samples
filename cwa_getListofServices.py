...
# On success, this script will produce service ids and their names.
#################################################################################################################
import json
import requests
import configparser
...
# create logger
logger = logging.getLogger("cwa_getListofServices")
...
logger.info("cwa_getListofServices: Current working Directory is " + Current_Working_Dir)
logger.info("cwa_getListofServices: Checking if config file present in current directory")

config_file = Path(Current_Working_Dir+'/config.ini')

# Checking if config file present
if not config_file.is_file():
    logger.error("cwa_getListofServices: File config.ini not found in current working directory, place config.ini "
                 "fle in directory \n " + Current_Working_Dir)
    exit()
else :
    logger.info("cwa_getListofServices: Configfile found in directory "+Current_Working_Dir)
...
def get_list_of_services():
    ...
    try:
        logger.info("get_list_of_services(): Hitting http request to get list of services ")
        service_list_response = requests.get(services_url, headers=auth_headers)
        if service_list_response.status_code == 200:
            logger.info("get_list_of_services(): List of services fetched successfully "
                        + str(service_list_response.status_code))

            service_list_json_array = json.loads(service_list_response.text)
            for service in service_list_json_array:
                service_id = service['serviceid']
                service_name = service['servicename']
                print("service id :: " + service_id + "  service name :: " + service_name)
        else:
